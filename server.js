...
const SETTINGS_COLLECTION_NAME = "user_settings";
...
async function connectMongo() {
  ...
  try {
    ...
    userSettingsCollection = db.collection(SETTINGS_COLLECTION_NAME);
...
app.post("/api/log/skip", async (req, res) => {
  try {
...
app.get("/api/moods/:user_id", async (req, res) => {
  try {
    const { user_id } = req.params;
    let logs = [];
    let source = "local";
